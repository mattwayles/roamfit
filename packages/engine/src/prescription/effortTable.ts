/**
 * §5.4 effort table. Several cells are stated as ranges in spec.md (sets "2-3", reps "12-15",
 * etc.) — a `SessionEntry` needs one concrete number, so this file fixes a deterministic pick
 * within each range and documents the choice (STATUS-2-engine.md "Ambiguities"):
 ...
 *     finisher slot (§5.5: "supersets + one finisher" at `hard`); every other `hard`-effort
...
export const EFFORT_TABLE: Record<Effort, EffortRow> = {
  easy: { sets: 3, reps: 15, restSec: 60, tempoSec: 3 },
  normal: { sets: 3, reps: 12, restSec: 45, tempoSec: 3 },
 ...
 *  rather than the reps table — the effort still governs sets/rest/tempo. */
export function effortRowFor(effort: Effort): EffortRow {
