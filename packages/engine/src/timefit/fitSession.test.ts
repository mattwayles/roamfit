...
describe('§5.1 step 6 time fit', () => {
  it('trims a required entry before dropping it, and drops it if even 1 set does not fit', () => {
    // Each entry costs 250s at its 1-set floor (10*3 + 100 + 120); three floors (750s) exceed the
    // 594s ceiling, but two (500s) fit — so exactly one of the three required entries is dropped.
    const slots = [
      { required: true, entry: entry('a', 3, 100) },
      { required: true, entry: entry('b', 3, 100) },
      { required: true, entry: entry('c', 3, 100) },
    ];
    ...
    // 500s the two of them cost together leaves no further room to grow) and 'c' is dropped,
  ...
  it('trims an optional entry to make room once the required entry ahead of it is settled', () => {
    const slots = [
      { required: true, entry: entry('req1', 3) }, // 345s — comfortably required, kept in full
      { required: false, entry: entry('opt1', 3, 80) }, // 450s full, too big alongside req1
    ];
    const result = fitMainEntries(slots, 15, 180, 180); // budget = (15-3-3)*60=540s, ceiling=594s
  ...
  it('reports whether the total estimate lands within +/-10% of target', () => {
    const slots = [{ required: true, entry: entry('solo', 3, 390) }]; // sets*(30+390)+120 = 1380s
