...
| 6 | Media ladder, LLM proxy, Firebase sync, HealthKit | in progress — approval gate cleared 2026-08-31. Serialized tracks. **`6a-figures` done** — verified `b827ad4` after four review rounds (see verification log). **`6b-media-ladder` done** — verified `fba0a93`. `6c-llm-proxy`, `6d-sync-health`, `6e-video-cli` not started. |
...
| 2 | **CLOSED by user decision, 2026-08-31.** `bw-inverted-row` was `anchor: pullup-bar` → `anchor_class: bodyweight_bearing` (off by default, effort-capped). User: "we can relax the classification. It doesn't need to be too strict." Its own setup cue supports this — *"Walk the feet in to make it easier"* is the partial-support argument. Reclassified this one exercise rather than changing `migrate_library.py`'s `pullup-bar → bodyweight_bearing` mapping rule, which stays correct for actual pull-ups. | 1B | closed — user decision |
...
| 33 | **Warm-up content gap, created by the contraindication tagging.** Tagging `wu-shoulder-passthrough` and `wu-arm-circles` `shoulder_overhead` leaves a shoulder-injured user with **zero shoulder-relevant warm-ups** — the warm-up pool is only 9 records and those are 2 of its 3 upper-body entries. Deliberately accepted rather than under-tagging a safety filter. Needs 3-4 new shoulder-safe upper-body warm-ups. Related to #3. | orchestrator, at sign-off | content pass, pre-launch |
