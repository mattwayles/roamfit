...
| 7 | Airplane-mode acceptance gate + instrumentation + polish | **partially done** — `2ea9062`+`64befe2`. §15 instrumentation, Settings screen, disclaimer gate and a store-level §11.6 harness landed; **the real airplane-mode gate on a physical device is NOT done and needs a human.** Pre-gate tracks — pre-gate tracks `6f-deploy` (`7ed9f15`) and `6g-warmups` (`dbb27db`) done first, so the gate tests a deployable build and the shipped content. |
...
| 33 | **CLOSED — track `6g-warmups`, verified `dbb27db`.** 4 new genuinely shoulder-safe warm-ups (`wu-scap-push-up`, `wu-band-external-rotation`, `wu-thread-the-needle`, `wu-band-row`). Warm-up pool 9 -> 13; shoulder-specific `focus: upper` survivors of a `shoulder_overhead` limitation **1 -> 5**, all 5 also surviving `shoulder_horizontal` (independently re-measured by the orchestrator). Library 200 -> 204 records, 204/204 figure coverage. | orchestrator, at sign-off | closed — 6g |
| 34 | **CLOSED — track `6f-deploy`, verified `e2daca4`.** `app/src/lib/llmProxyClient.ts` implements `LlmProxyCaller`, wired into `opportunisticSync.ts`. Orchestrator found and closed a real residual gap the agent flagged on itself: `HomeScreen`'s fire-and-forget chain had no `.catch()`, so a rejection became an unhandled rejection. Added the catch plus a screen-level test (`HomeScreen.syncIsolation.test.tsx`) proving Home still renders local data when the whole sync chain rejects. | 6c-llm-proxy | closed — 6f |
...
| 41 | **CLOSED — track `6f-deploy`, verified `7ed9f15`.** Real esbuild build bundling the workspace-local `@roamfit/engine`/`@roamfit/data` while leaving npm deps external; `firebase.json` predeploy runs typecheck+build. Orchestrator ran the build and loaded `lib/index.js` in node — all three handlers export as callables (not a bare `tsc` exit 0). Operator runbook: `docs/RUNBOOK-functions-deploy.md`. | 6d-sync-health | closed — 6f |
| 44 | **No guardrail for a stale `app/ios/`.** Wave 6 added four native deps (`react-native-webview`, HealthKit, `expo-location`, `react-native-svg`) and nobody re-ran `expo prebuild`/`pod install`, so the app red-screened on boot with `RNCWebViewModule not found` — meaning no Wave 4-6 device claim on this machine was ever real. Fixed locally, but `app/ios/` is gitignored, so nothing prevents recurrence. Needs `docs/RUNBOOK-ios-build.md` or a CI check that fails when native deps change without a rebuild. | 7-acceptance | pre-launch |
...
| 7 | "5-session gate + completion-atomicity test, both mutation-verified" | **One of the two was not.** Removing `completion.ts`'s `db.transaction` wrapper entirely (verified `db.transaction` count 1 -> 0) left all 112 store tests green, including the atomicity test. Its three assertions all hold either way, because nothing after the forced throw runs regardless; the distinguishing assertion — per-entry exercise state written *before* the throw — was described in the test's own comment but never written. Fixed in `64befe2`; now fails when the transaction is removed. **Twelfth entry in this log, and the wave whose job was to not become it.** The 5-session gate test itself did survive mutation. |
| 7 (credit) | "reproduced a red-screen crash; nothing in Waves 4-6 could have run on a device" | **Confirmed, and the most valuable find of the project.** `Podfile.lock` now carries `react-native-webview` 13.16.1, `ReactNativeHealthkit` 14.1.0, `RNSVG`, `ExpoLocation` — the Wave 6 native deps that had never been linked. Simulator screenshots verified by the orchestrator: the disclaimer gate and the full Home dashboard (Today card, Quick Session ~7min, non-punishing week dots, Next Unlock, live progression board at Level 3 of 9 / 3 of 8 / 3 of 7) render on the rebuilt binary. First genuine device-level evidence in the project. Caveat: `app/ios/` is gitignored, so the fix is a local build artifact — a stale local `ios/` reproduces it. |
