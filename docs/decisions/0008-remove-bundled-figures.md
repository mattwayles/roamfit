...
## Follow-up, since done

`react-native-svg` was the only consumer of the figures and is now removed from
`app/package.json`. It was not a transitive dependency of anything else (`npm ls` confirms a
single edge, from `app`). `app/ios/` is gitignored and regenerated by `expo prebuild`, so nothing
native is tracked — but **anyone with an existing dev client must rebuild it** (`./scripts/dev.sh
--build`), because the installed binary still contains the RNSVG native module and its JS side is
gone. Metro alone will not reconcile that.
